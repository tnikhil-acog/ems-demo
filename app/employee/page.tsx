...
export default function EmployeeDashboard() {
  ...
  // Get current user's allocations from project_allocations table
  const allocations = data.project_allocations || [];
  const currentUserAllocation = allocations.filter(
    (a) => a.emp_id === "EMP001"
  );

  // Get projects assigned to current user
  const projectsForUser = data.projects.filter((p) =>
    currentUserAllocation.some((alloc) => alloc.project_id === p.id)
  );
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {projectsForUser.map((project) => {
                    // Get allocation percentage for this project
                    const alloc = currentUserAllocation.find(
                      (a) => a.project_id === project.id
                    );
                    const allocation = alloc?.allocation_percentage || 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3">
                          {alloc?.is_lead ? "Lead" : "Senior Engineer"}
                        </td>
                        <td className="py-3">
