...
export default function HRDashboard() {
  // Use DataLoader to centralize loading UI and data fetching
  return (
    <DashboardLayout role="hr" title="HR Dashboard" currentPath="/hr">
      <DataLoader>
        {(data) => {
            ...
            if (emp.skills) {
              emp.skills.forEach((skill) => {
                if (!skillsMap.has(skill.technology)) {
                  skillsMap.set(skill.technology, {
                    employees: [],
                    ratings: [],
                  });
                }
          ...
          const topSkills: SkillWithEmployees[] = Array.from(
            skillsMap.entries()
          )
            .map(([skill, data]) => ({
              skill,
          ...
          allocations.forEach((alloc: any) => {
            ...
            if (dept) {
              deptAllocMap.set(
                dept,
                (deptAllocMap.get(dept) || 0) + alloc.allocation_percentage
              );
            }
          });

          // Step 3: Build stats O(n)
          const departmentStats: DepartmentStats[] = Array.from(
            departmentsMap.entries()
          ).map(([dept, emps]) => {
            const totalAllocation = deptAllocMap.get(dept) || 0;
            ...
            return {
              ...
              onBench: emps.filter((e) => !e.skills || e.skills.length === 0)
                .length,
          ...
          const statsData = [
            ...
            },
            {
              ...
              icon: Users,
              variant:
                onBench > 10 ? ("warning" as const) : ("default" as const),
            },
            {
              title: "Exits Pending",
              value: exitPipeline,
              icon: UserX,
              variant:
                exitPipeline > 5 ? ("danger" as const) : ("default" as const),
            },
          ];
                ...
                <CardContent className="space-y-4">
                  <div>
                    <p className="text-sm text-muted-foreground mb-2">
                      Submit a consolidated report of your work across HR
